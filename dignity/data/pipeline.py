...
class TransactionPipeline:
    ...
    def compute_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'price' in df.columns:
            result['volatility'] = self.signal_processor.volatility(
                df['price'].values
            )
    ...
    def create_sequences(self,
        ...
        if y is not None:
            if y.ndim == 1:
                ...
                y_seq = np.array([y[i * stride + self.seq_len - 1] 
                                 for i in range(n_sequences)])
            else:
                # Regression targets (take window after sequence)
                pred_len = y.shape[1] if y.ndim > 1 else 1
                y_seq = np.array([y[i * stride + self.seq_len:
                                    i * stride + self.seq_len + pred_len]
                                 for i in range(n_sequences)])
